...
int main ()
{
	...
	*p_item_manager_1 = *p_item_manager_1 = *p_item_manager_1;
	...
	if(is_inventory_print_sorted || is_location_print_sorted)
	{
		...
		cout << "    -10 marks if they are in the ItemManager.cpp file" << endl;
	}
                             ...
                             bool is_finished_without_crash)
{
	...
	if(correct_location_default_fraction == 1.0)
		mark += 1;
	if(correct_inventory1_fraction == 1.0)
		mark += 1;
	mark += (int)(correct_location1_fraction * 3);
	if(correct_count_copy)
		mark += 1;
	if(correct_count_assign)
		mark += 1;
	...
	if(correct_location_self_fraction    == 1.0 &&
	   correct_location_chained_fraction == 1.0)
	{
		mark += 1;
	}
	...
	int inventory2_mark = (int)(correct_inventory2_fraction * 4);
	if(inventory2_mark > 0)
		mark += inventory2_mark;
	if(correct_location2_fraction == 1.0)
		mark += 1;
	int inventory3_mark = (int)(correct_inventory3_fraction * 3);
	if(inventory3_mark > 0)
		mark += inventory3_mark;
	int location3_mark = (int)(correct_location3_0_fraction    * 2 +
	                           correct_location3_1_fraction    * 2 +
	                           correct_location3_rest_fraction * 2 - 3);
	if(location3_mark > 0)
		mark += location3_mark;
