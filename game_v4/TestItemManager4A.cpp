...
int main ()
{
	testHelperStartup("TestItemManager4A", COMPILE_AND_START_MARKS, TOTAL_MARKS);
	...
	if(correct_count_count == 3)
		mark += 1;
	mark += correct_score_count;
	int inventory_mark = (int)(correct_inventory1_fraction * 2 +
	                           correct_inventory2_fraction * 2 +
	                           correct_inventory3_fraction * 2 - 2);
	if(inventory_mark > 0)
		mark += inventory_mark;
	int location_mark = (int)(correct_location1_fraction      * 2 +
	                          correct_location2_fraction      * 2 +
	                          correct_location3_0_fraction    * 1 +
	                          correct_location3_1_fraction    * 1 +
	                          correct_location3_rest_fraction * 1 - 3);
	if(location_mark > 0)
		mark += location_mark;
                          ...
                          bool is_print_incorrect)
{
	...
	for(unsigned int i = 0; i < ITEM_COUNT_CORRECT; i++)
	{
		...
		{
			if(result.missing_count == 0 && is_print_incorrect)
